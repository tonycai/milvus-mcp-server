...
class MilvusConnector:
    ...
    async def create_collection(
        ...
        try:
            ...
            # Handle id_type conversion from string to DataType enum
            ...
            if isinstance(id_type, str):
                ...
                elif id_type == "VARCHAR":
                    id_type = DataType.VARCHAR
                elif id_type == "FLOAT":
                    id_type = DataType.FLOAT
                elif id_type == "STRING":
                    id_type = DataType.STRING
            
            # Process other_fields to ensure proper data types
            ...
            # Create collection with minimal required parameters
            # Only use dimension if it's specified, otherwise use simplest creation
            if "dimension" in schema:
                # Use full parameter set
            ...
            if index_params:
                try:
                    ...
                    print(f"Warning: Failed to create index: {str(index_error)}")
    ...
    async def insert_data(
        ...
        try:
            ...
            # Check schema to determine expected data types
            try:
                collection_info = self.client.describe_collection(collection_name)
                schema_fields = collection_info.get("schema", {}).get("fields", [])
                
                # Track vector fields to properly format them
                vector_fields = []
                for field in schema_fields:
                    if field.get("type") in ["FLOAT_VECTOR", "BINARY_VECTOR"]:
                        vector_fields.append(field.get("name"))
                
                # Convert numpy arrays or lists to appropriate format for vector fields
                import numpy as np
                for field_name in vector_fields:
                    if field_name in processed_data:
                        vectors = processed_data[field_name]
                        
                        # If this is a numpy array, convert to list
                        if isinstance(vectors, np.ndarray):
                            processed_data[field_name] = vectors.tolist()
                        
                        # Ensure proper nesting structure for vectors
                        if vectors and not isinstance(vectors[0], list):
                            # If this is a single vector as a flat list, wrap it
                            processed_data[field_name] = [vectors]
            except Exception as schema_error:
                # If we can't read the schema, continue with original data
                print(f"Warning: Could not process schema: {str(schema_error)}")
            
            # Try insert with processed data
            result = self.client.insert(collection_name=collection_name, data=processed_data)
            return result
        except Exception as e:
            # If first attempt fails, try with minimal processing
            try:
                print(f"Warning: First insert attempt failed: {str(e)}")
                # Try with just the vector field and minimal data
                if "vector" in data:
                    minimal_data = {"vector": data["vector"]}
                    result = self.client.insert(collection_name=collection_name, data=minimal_data)
                    return result
...
async def milvus_create_collection(
    ...
    Args:
        ...
        index_params: Optional index parameters
    """
    # Convert id_type from string to DataType if needed
    if "id_type" in collection_schema and isinstance(collection_schema["id_type"], str):
        if collection_schema["id_type"] == "INT64":
            collection_schema["id_type"] = DataType.INT64
        elif collection_schema["id_type"] == "INT32":
            collection_schema["id_type"] = DataType.INT32
        elif collection_schema["id_type"] == "VARCHAR":
            collection_schema["id_type"] = DataType.VARCHAR
        elif collection_schema["id_type"] == "FLOAT":
            collection_schema["id_type"] = DataType.FLOAT
